...
struct sc_pkcs11_object_ops {
  ...
  CK_RV(*can_do)(struct sc_pkcs11_session*, struct sc_pkcs11_object*,
      CK_MECHANISM_TYPE,
      int ck_type);
  CK_RV(*encrypt)(struct sc_pkcs11_session*, struct sc_pkcs11_object*,
      CK_MECHANISM_PTR,
      CK_BYTE_PTR data, CK_ULONG data_len, CK_BYTE_PTR p_enc, CK_ULONG_PTR p_enc_len);
...
struct sc_pkcs11_framework_ops {
  ...
  CK_RV (*unblock_pin)(struct sc_pkcs11_card *, void *,
        CK_CHAR_PTR, CK_ULONG,
        CK_CHAR_PTR, CK_ULONG);
...
struct sc_pkcs11_card {
  ...
  struct sc_pkcs11_mechanism_type *mechanisms[20];
...
struct sc_pkcs11_mechanism_type {
  ...
  CK_RV      (*md_update)(sc_pkcs11_operation_t *,
          CK_BYTE_PTR, CK_ULONG);
  /* CLCO 20/05/2010 : modification pour gestion IAS, indiquer que le hash doit Ãªtre partiel si besoin */
...
struct sc_pkcs11_operation {
  ...
  void *      priv_data;
  unsigned char already_feed_once;
};
...
/* BPER 1381 - Solution C, récuprération du contexte Cryptoki (thread / global) */
extern sc_context_t * getCurContext(void);
extern sc_context_t * getCurContext_finalize(ULONG dwThreadID);
void strcpy_bp(u8 *dst, const char *src, size_t dstsize);
CK_RV sc_to_cryptoki_error(int rc, int reader);
/* BPER 1381 - Solution C, ajout parametre du contexte - Debut */
void sc_pkcs11_print_attrs(const char *file, unsigned int line, const char *function,
    const char *info, CK_ATTRIBUTE_PTR pTemplate, CK_ULONG ulCount, struct sc_context *);
#define dump_template(info, pTemplate, ulCount, ctx) \
    sc_pkcs11_print_attrs(__FILE__, __LINE__, __FUNCTION__, \
        info, pTemplate, ulCount, ctx)
/* BPER 1381 - Solution C, ajout parametre du contexte - Fin */

/* Slot and card handling functions */
CK_RV card_initialize(int reader, struct sc_pkcs11_slot *, struct sc_pkcs11_card *);
CK_RV card_detect_all(void);
CK_RV __card_detect_all(struct sc_context *, int);
CK_RV card_detect(int reader, struct sc_context *);
CK_RV card_removed(int reader, unsigned long dwThreadID);
CK_RV slot_initialize(int id, struct sc_pkcs11_slot *);
CK_RV slot_get_slot(int id, struct sc_pkcs11_slot **, sc_context_t *pcontext);
CK_RV slot_get_token(int id, struct sc_pkcs11_slot **, int bCardPresence);
CK_RV slot_token_removed(int id, struct sc_context *);
...
CK_RV pool_find_and_delete(struct sc_pkcs11_pool *, CK_ULONG, void **, struct sc_context *);
...
CK_RV sc_pkcs11_close_all_sessions(CK_SLOT_ID, struct sc_context *);
/* CLCO 26/05/2010 : Ajout d'une fonction permettant de tester la validité de la session */
CK_RV is_session_valid(struct sc_pkcs11_session *);
/* CLCO 26/05/2010 : fin */
...
CK_RV sc_pkcs11_md_update(struct sc_pkcs11_session *, CK_BYTE_PTR, CK_ULONG, CK_BBOOL);
...
CK_RV sc_pkcs11_encr_init(struct sc_pkcs11_session*, CK_MECHANISM_PTR, struct sc_pkcs11_object*, CK_KEY_TYPE);
CK_RV sc_pkcs11_encr(struct sc_pkcs11_session* session, CK_BYTE_PTR pData, CK_ULONG ulDataLen, CK_BYTE_PTR pEncryptedData, CK_ULONG_PTR pulEncryptedDataLen);
...
CK_RV sc_pkcs11_unregister_all_mechanisms(struct sc_pkcs11_card *);
/* CLCO 25/05/2010 : fin */
#ifdef ENABLE_OPENSSL
...
CK_RV sc_pkcs11_gen_keypair_soft(struct sc_context *context, CK_KEY_TYPE keytype, CK_ULONG keybits,
  struct sc_pkcs15_prkey *privkey, struct sc_pkcs15_pubkey *pubkey);
CK_RV sc_pkcs11_verify_data(struct sc_context * ctx, const unsigned char *pubkey, int pubkey_len,
...
CK_RV sc_pkcs11_encrypt_data(struct sc_context* pcontext, const unsigned char* pubkey, int pubkey_len,
...
extern struct sc_pkcs11_pool * getPoolTable(void);
extern struct sc_pkcs11_pool * getPoolTable_ThrID(unsigned long thrID);
#ifdef __cplusplus
}
#endif
