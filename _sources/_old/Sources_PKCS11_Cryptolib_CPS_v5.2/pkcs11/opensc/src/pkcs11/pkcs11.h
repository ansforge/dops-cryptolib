...
/* AROC 08/04/2013 - Ajout des fonctions pour la gestion de la mise à jour des cartes */
_CK_DECLARE_FUNCTION (C_StartUpdate, (ck_session_handle_t hSession ));
_CK_DECLARE_FUNCTION (C_EndUpdate, (ck_session_handle_t hSession ));
_CK_DECLARE_FUNCTION (C_KeepAlive, (ck_session_handle_t hSession ));
_CK_DECLARE_FUNCTION (C_TransmitMessage, 
          (ck_session_handle_t hSession, 
...
/* AROC 08/04/2013 - Ajout des fonctions pour la gestion de la mise à jour des cartes */
    ...
    /* Ajout des fonctions pour la gestion de la mise à jour des cartes */
    CK_C_StartUpdate C_StartUpdate;
    CK_C_EndUpdate C_EndUpdate;
    CK_C_TransmitMessage C_TransmitMessage;
    CK_C_KeepAlive C_KeepAlive;
    /* Ajout des fonctions pour la gestion de la mise à jour des cartes */
	};

  /* CLCO 06/07/2010 : Adaptation ASIP des traces */
  CK_RV IC_Initialize(void *init_args);
  CK_RV IC_Finalize(void *reserved);
  CK_RV IC_GetInfo(struct ck_info *info);
  CK_RV IC_GetFunctionList(struct ck_function_list **function_list);

  CK_RV IC_GetSlotList(unsigned char token_present, ck_slot_id_t *slot_list,
    unsigned long *count);
  CK_RV IC_GetSlotInfo(ck_slot_id_t slot_id, struct ck_slot_info *info);
  CK_RV IC_GetTokenInfo(ck_slot_id_t slot_id, struct ck_token_info *info);
  CK_RV IC_WaitForSlotEvent(ck_flags_t flags, ck_slot_id_t *slot, void *reserved);
  CK_RV IC_GetMechanismList(ck_slot_id_t slot_id,
    ck_mechanism_type_t *mechanism_list,
    unsigned long *count);
  CK_RV IC_GetMechanismInfo(ck_slot_id_t slot_id, ck_mechanism_type_t type,
  struct ck_mechanism_info *info);
  CK_RV IC_InitToken(ck_slot_id_t slot_id, unsigned char *pin,
    unsigned long pin_len, unsigned char *label);
  CK_RV IC_InitPIN(ck_session_handle_t session, unsigned char *pin,
    unsigned long pin_len);
  CK_RV IC_SetPIN(ck_session_handle_t session, unsigned char *old_pin,
    unsigned long old_len, unsigned char *new_pin,
    unsigned long new_len);

  CK_RV IC_OpenSession(ck_slot_id_t slot_id, ck_flags_t flags,
    void *application, ck_notify_t notify,
    ck_session_handle_t *session);
  CK_RV IC_CloseSession(ck_session_handle_t session);
  CK_RV IC_CloseAllSessions(ck_slot_id_t slot_id);
  CK_RV IC_GetSessionInfo(ck_session_handle_t session,
  struct ck_session_info *info);
  CK_RV IC_GetOperationState(ck_session_handle_t session,
    unsigned char *operation_state,
    unsigned long *operation_state_len);
  CK_RV IC_SetOperationState(ck_session_handle_t session,
    unsigned char *operation_state,
    unsigned long operation_state_len,
    ck_object_handle_t encryption_key,
    ck_object_handle_t authentiation_key);
  CK_RV IC_Login(ck_session_handle_t session, ck_user_type_t user_type,
    unsigned char *pin, unsigned long pin_len);
  CK_RV IC_Logout(ck_session_handle_t session);

  CK_RV IC_CreateObject(ck_session_handle_t session,
  struct ck_attribute *templ,
    unsigned long count, ck_object_handle_t *object);
  CK_RV IC_CopyObject(ck_session_handle_t session, ck_object_handle_t object,
  struct ck_attribute *templ, unsigned long count,
    ck_object_handle_t *new_object);
  CK_RV IC_DestroyObject(ck_session_handle_t session,
    ck_object_handle_t object);
  CK_RV IC_GetObjectSize(ck_session_handle_t session,
    ck_object_handle_t object,
    unsigned long *size);
  CK_RV IC_GetAttributeValue(ck_session_handle_t session,
    ck_object_handle_t object,
  struct ck_attribute *templ,
    unsigned long count);
  CK_RV IC_SetAttributeValue(ck_session_handle_t session,
    ck_object_handle_t object,
  struct ck_attribute *templ,
    unsigned long count);
  CK_RV IC_FindObjectsInit(ck_session_handle_t session,
  struct ck_attribute *templ,
    unsigned long count);
  CK_RV IC_FindObjects(ck_session_handle_t session,
    ck_object_handle_t *object,
    unsigned long max_object_count,
    unsigned long *object_count);
  CK_RV IC_FindObjectsFinal(ck_session_handle_t session);

  CK_RV IC_EncryptInit(ck_session_handle_t session,
  struct ck_mechanism *mechanism,
    ck_object_handle_t key);
  CK_RV IC_Encrypt(ck_session_handle_t session,
    unsigned char *data, unsigned long data_len,
    unsigned char *encrypted_data,
    unsigned long *encrypted_data_len);
  CK_RV IC_EncryptUpdate(ck_session_handle_t session,
    unsigned char *part, unsigned long part_len,
    unsigned char *encrypted_part,
    unsigned long *encrypted_part_len);
  CK_RV IC_EncryptFinal(ck_session_handle_t session,
    unsigned char *last_encrypted_part,
    unsigned long *last_encrypted_part_len);

  CK_RV IC_DecryptInit(ck_session_handle_t session,
  struct ck_mechanism *mechanism,
    ck_object_handle_t key);
  CK_RV IC_Decrypt(ck_session_handle_t session,
    unsigned char *encrypted_data,
    unsigned long encrypted_data_len,
    unsigned char *data, unsigned long *data_len);
  CK_RV IC_DecryptUpdate(ck_session_handle_t session,
    unsigned char *encrypted_part,
    unsigned long encrypted_part_len,
    unsigned char *part, unsigned long *part_len);
  CK_RV IC_DecryptFinal(ck_session_handle_t session,
    unsigned char *last_part,
    unsigned long *last_part_len);

  CK_RV IC_DigestInit(ck_session_handle_t session,
  struct ck_mechanism *mechanism);
  CK_RV IC_Digest(ck_session_handle_t session,
    unsigned char *data, unsigned long data_len,
    unsigned char *digest,
    unsigned long *digest_len);
  CK_RV IC_DigestUpdate(ck_session_handle_t session,
    unsigned char *part, unsigned long part_len);
  CK_RV IC_DigestKey(ck_session_handle_t session, ck_object_handle_t key);
  CK_RV IC_DigestFinal(ck_session_handle_t session,
    unsigned char *digest,
    unsigned long *digest_len);

  CK_RV IC_SignInit(ck_session_handle_t session,
  struct ck_mechanism *mechanism,
    ck_object_handle_t key);
  CK_RV IC_Sign(ck_session_handle_t session,
    unsigned char *data, unsigned long data_len,
    unsigned char *signature,
    unsigned long *signature_len);
  CK_RV IC_SignUpdate(ck_session_handle_t session,
    unsigned char *part, unsigned long part_len);
  CK_RV IC_SignFinal(ck_session_handle_t session,
    unsigned char *signature,
    unsigned long *signature_len);
  CK_RV IC_SignRecoverInit(ck_session_handle_t session,
  struct ck_mechanism *mechanism,
    ck_object_handle_t key);
  CK_RV IC_SignRecover(ck_session_handle_t session,
    unsigned char *data, unsigned long data_len,
    unsigned char *signature,
    unsigned long *signature_len);

  CK_RV IC_VerifyInit(ck_session_handle_t session,
  struct ck_mechanism *mechanism,
    ck_object_handle_t key);
  CK_RV IC_Verify(ck_session_handle_t session,
    unsigned char *data, unsigned long data_len,
    unsigned char *signature,
    unsigned long signature_len);
  CK_RV IC_VerifyUpdate(ck_session_handle_t session,
    unsigned char *part, unsigned long part_len);
  CK_RV IC_VerifyFinal(ck_session_handle_t session,
    unsigned char *signature,
    unsigned long signature_len);
  CK_RV IC_VerifyRecoverInit(ck_session_handle_t session,
  struct ck_mechanism *mechanism,
    ck_object_handle_t key);
  CK_RV IC_VerifyRecover(ck_session_handle_t session,
    unsigned char *signature,
    unsigned long signature_len,
    unsigned char *data,
    unsigned long *data_len);

  CK_RV IC_DigestEncryptUpdate(ck_session_handle_t session,
    unsigned char *part, unsigned long part_len,
    unsigned char *encrypted_part,
    unsigned long *encrypted_part_len);
  CK_RV IC_DecryptDigestUpdate(ck_session_handle_t session,
    unsigned char *encrypted_part,
    unsigned long encrypted_part_len,
    unsigned char *part,
    unsigned long *part_len);
  CK_RV IC_SignEncryptUpdate(ck_session_handle_t session,
    unsigned char *part, unsigned long part_len,
    unsigned char *encrypted_part,
    unsigned long *encrypted_part_len);
  CK_RV IC_DecryptVerifyUpdate(ck_session_handle_t session,
    unsigned char *encrypted_part,
    unsigned long encrypted_part_len,
    unsigned char *part,
    unsigned long *part_len);

  CK_RV IC_GenerateKey(ck_session_handle_t session,
  struct ck_mechanism *mechanism,
  struct ck_attribute *templ,
    unsigned long count,
    ck_object_handle_t *key);
  CK_RV IC_GenerateKeyPair(ck_session_handle_t session,
  struct ck_mechanism *mechanism,
  struct ck_attribute *public_key_template,
    unsigned long public_key_attribute_count,
  struct ck_attribute *private_key_template,
    ...
    ck_object_handle_t *public_key,
    ck_object_handle_t *private_key);
  CK_RV IC_WrapKey(ck_session_handle_t session,
  struct ck_mechanism *mechanism,
    ck_object_handle_t wrapping_key,
    ck_object_handle_t key,
    unsigned char *wrapped_key,
    unsigned long *wrapped_key_len);
  CK_RV IC_UnwrapKey(ck_session_handle_t session,
  struct ck_mechanism *mechanism,
    ck_object_handle_t unwrapping_key,
    unsigned char *wrapped_key,
    unsigned long wrapped_key_len,
  struct ck_attribute *templ,
    unsigned long attribute_count,
    ck_object_handle_t *key);
  CK_RV IC_DeriveKey(ck_session_handle_t session,
  struct ck_mechanism *mechanism,
    ck_object_handle_t base_key,
  struct ck_attribute *templ,
    unsigned long attribute_count,
  ...
  CK_RV IC_SeedRandom(ck_session_handle_t session, unsigned char *seed,
    unsigned long seed_len);
  CK_RV IC_GenerateRandom(ck_session_handle_t session,
    unsigned char *random_data,
    unsigned long random_len);

  CK_RV IC_GetFunctionStatus(ck_session_handle_t session);
  CK_RV IC_CancelFunction(ck_session_handle_t session);
  /* CLCO 06/07/2010 : Fin  */

/* Ajout des fonctions pour la gestion de la mise à jour des cartes */
  CK_RV IC_StartUpdate(ck_session_handle_t hSession );
  CK_RV IC_EndUpdate(ck_session_handle_t hSession );
  CK_RV IC_KeepAlive(ck_session_handle_t hSession );
  CK_RV IC_TransmitMessage(ck_session_handle_t hSession, 
                          unsigned char *pbMessage,
                          unsigned long  szMessage,
                          unsigned char *pbResponse,
                          unsigned long *pszResponse,
...
#define CKR_VENDOR_DEFINED			(1UL << 31)
  ...
  /* JTAU 16/11/2010 : Code erreur ASIP dans le cas ou la carte est dans un état inconnu suite à une mise à jour */
#define CKR_ASIP_BAD_EF_ACTUA      (CKR_VENDOR_DEFINED + 1)
#define CKR_ASIP_REINIT_NEEDED    (CKR_VENDOR_DEFINED + 2)
/* JTAU 16/11/2010 : fin*/
