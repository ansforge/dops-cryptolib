...
static void    sc_pkcs11_print_attr(const char *, unsigned int,
        ...
        CK_ATTRIBUTE_PTR, struct sc_context *);
...
void sc_pkcs11_print_attrs(const char *file, unsigned int line,
      const char *function,
      const char *info,
      CK_ATTRIBUTE_PTR pTemplate, CK_ULONG ulCount, struct sc_context *pcontext /* BPER 1381 - Solution C */)
{
  ...
  if (ulCount == 0) {
    sc_do_log(pcontext, SC_LOG_TYPE_DEBUG,
      file, line, function,
      "%s: empty template\n",
      info);
    return;
  ...
  while (ulCount--)
    sc_pkcs11_print_attr(file, line, function,
        info, pTemplate++, pcontext);

}

static void sc_pkcs11_print_attr(const char *file, unsigned int line,
      const char *function,
      const char *info, CK_ATTRIBUTE_PTR attr, struct sc_context *pcontext) /* BPER 1381 - Solution C */
{
  struct fmap  *fm;
  const char *  value;
