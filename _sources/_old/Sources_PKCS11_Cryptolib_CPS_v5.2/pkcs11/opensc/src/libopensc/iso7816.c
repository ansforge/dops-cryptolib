...
static const struct sc_card_error iso7816_errors[] = {
  ...
  { 0x6881, SC_ERROR_BAD_CHANNEL,  "Logical channel not supported" },
  ...
  { 0x6984, /*SC_ERROR_CARD_CMD_FAILED*/SC_ERROR_NOT_ALLOWED,  "Referenced data invalidated" },
             ...
             unsigned long flags)
{
  ...
  apdu.resplen = count;
  apdu.resp = recvbuf;
  if (flags&CPS2TER_FLAG) { apdu.cla = 0xA0; }
  ...
  SC_FUNC_RETURN(card->ctx, 3, (int)apdu.resplen);
}
             ...
             unsigned long flags)
{
  ...
  /* CLCO 06/07/2010 : Suppression warning compilation  */
  apdu.p2 = (u8)(flags & SC_RECORD_EF_ID_MASK) << 3;
  /* CLCO 06/07/2010 : Fin  */
              ...
              unsigned long flags)
{
  ...
  SC_TEST_RET(card->ctx, sc_check_sw(card, apdu.sw1, apdu.sw2),
        "Card returned error");
  SC_FUNC_RETURN(card->ctx, 3, (int)count);
}
           ...
           const u8 *buf, size_t buflen)
{
  ...
  if (ctx->debug >= 3) { sc_debug(ctx, "processing FCI bytes\n"); }

  tag = sc_asn1_find_tag(ctx, p, len, 0x83, &taglen);
  if (tag != NULL && taglen == 2) {
    file->id = (tag[0] << 8) | tag[1];
    if (ctx->debug >= 3) { sc_debug(ctx, "  File id: 0x%02X%02X\n", tag[0], tag[1]); }
  }
  ...
  if (tag != NULL && taglen >= 2) {
    ...
    if (ctx->debug >= 3) { sc_debug(ctx, "  File Size: %d\n", size); }
    file->size = size;
  }

  if (tag == NULL && file->size == 0) {
  ...
  if (tag != NULL) {
    if (taglen > 0) {
      ...
      if (ctx->debug >= 3) { sc_debug(ctx, "  Shareable: %s\n", (byte & 0x40) ? "yes" : "no"); }
...
static int iso7816_get_response(sc_card_t *card, size_t *count, u8 *buf)
{
  ...
  *count = apdu.resplen;
  ...
  else if (apdu.sw1 == card->sw1_bytes_available)
...
static struct sc_card_operations iso_ops = {
  ...
  NULL, /* delete_record */
  NULL, /* compute_hash */
  NULL, /* get_aid_pkcs15_from_atr */
  NULL, /* get_pin_counter */
  NULL, /* get_model */
  NULL, /* verify_update */
  NULL, /* is_visible */
  NULL, /* is_valid */
  NULL, /* cps2ter_select_file*/
  NULL, /* AROC 08/04/2013 - start_exlusivity */
  NULL, /* AROC 08/04/2013 - end_exlusivity   */
  NULL, /* AROC 08/04/2013 - free_transmit    */
};
