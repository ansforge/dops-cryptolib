...
void sc_mk_apdu_sensitive(sc_apdu_t *source, sc_apdu_t *destination) {

  destination->cse = source->cse;
  destination->cla = source->cla;
  destination->ins = source->ins;
  destination->p1 = source->p1;
  destination->p2 = source->p2;
  destination->lc = source->lc;
  destination->le = source->le;
  destination->data = calloc(source->datalen, sizeof(u8));
  if (destination->data != NULL) {
    memset((void*)destination->data, '*', source->datalen);
    destination->datalen = source->datalen;
  }
  else {
    destination->datalen = 0;
  }
  destination->resp = calloc(source->resplen, sizeof(u8));
  if (destination->resp != NULL) {
    memcpy(destination->resp, source->resp, source->resplen);
    destination->resplen = source->resplen;
  }
  else {
    destination->resplen = 0;
  }
  destination->sensitive = source->sensitive;
  destination->control = source->control;
  destination->sw1 = source->sw1;
  destination->sw2 = source->sw2;
  destination->flags = source->flags;

}

void sc_free_apdu(sc_apdu_t *apdu) {
  free((void*)apdu->data);
  free(apdu->resp);
}
...
void sc_apdu_log(sc_context_t *ctx, sc_apdu_t *apdu, unsigned int active_protocol)
{
...
#ifndef _DEBUG
  if (apdu->sensitive == 1) {
    sc_mk_apdu_sensitive(apdu, &apdu_sensitive);
    apdu_log = &apdu_sensitive;
  }
  else
#endif // ! DEBUG
    apdu_log = apdu;

  sc_apdu_get_octets(ctx, apdu_log, &sbuf, &ssize, active_protocol);
  ...
  sc_hex_dump(ctx, sbuf, ssize, buf, blen);
...
static void do_select_cps_aid(sc_card_t *card)
{
  static const u8 cps_ias_aid[] = { 0x00, 0xA4, 0x04, 0x04, 0x08, 0x80, 0x25, 0x00, 0x00, 0x01, 0xFF, 0x01, 0x00, 0x00 };
  sc_apdu_t       AppIAS_apdu;
  u8              data[256];

  memset(data, 0, 256);
  memcpy(data, &(cps_ias_aid[5]), cps_ias_aid[4]);

  
  AppIAS_apdu.cse = SC_APDU_CASE_3_SHORT;
  AppIAS_apdu.cla = card->cla;
  AppIAS_apdu.ins = cps_ias_aid[1];
  AppIAS_apdu.p1 = cps_ias_aid[2];
  AppIAS_apdu.p2 = cps_ias_aid[3];
  AppIAS_apdu.datalen = cps_ias_aid[4];
  AppIAS_apdu.data = data;
  AppIAS_apdu.lc = cps_ias_aid[4];
  AppIAS_apdu.resplen = AppIAS_apdu.le = 0;
  AppIAS_apdu.resp = NULL;
  AppIAS_apdu.sensitive = AppIAS_apdu.control = 0;
  AppIAS_apdu.flags = 0;

  card->reader->ops->transmit(card->reader, card->slot, &AppIAS_apdu);
}
...
static int do_single_transmit(sc_card_t *card, sc_apdu_t *apdu)
{
  ...
  */
  ...
  if ((apdu->sw1 == 0x6E) || (apdu->sw1 == 0x6A && apdu->sw2 == 0x86)) {
    /* BPER (@@20160608-1359) - Re-selectionner l'appli IAS CPS ... */
    do_select_cps_aid(card);
  
    // et re-transmettre l'apdu.
    r = card->reader->ops->transmit(card->reader, card->slot, apdu);
    if (r != 0) {
      sc_error(ctx, "unable to transmit APDU");
      return r;
   ...
   */
  if (((apdu->sw1 == card->sw1_bytes_available) || (apdu->sw1 == card->sw1_cps2ter_bytes_available)) && (apdu->flags & SC_APDU_FLAGS_NO_GET_RESP) == 0) {
    if (apdu->le == 0) {
      ...
      do {
        ...
        if (apdu->sw1 == card->sw1_cps2ter_bytes_available) {
          card->cla = 0xA0;
