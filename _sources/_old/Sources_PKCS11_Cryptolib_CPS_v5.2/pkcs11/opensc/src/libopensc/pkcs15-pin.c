        ...
        const u8 ** buf, size_t *buflen)
{
  ...
  /* CLCO 02/06/2010 : lire les informations relatives au nombre d'essais */
  r = sc_pkcs15_get_pin_counter(p15card, &info); 
  if (r)
    return r;
  /* CLCO 02/06/2010 : fin */
       ...
       const u8 *pincode, size_t pinlen)
{
  ...
  /* CLCO 02/06/2010 : Gestion du nombre d'essais restants */
  if (!r) /* si pas d'erreur, il faut remettre le compteur à sa valeur max */
    pin->tries_left=pin->tries_max;
       ...
       const u8 *newpin, size_t newpinlen)
{
  ...
  data.pin1.len        = (int)oldpinlen;
  data.pin1.pad_char   = pin->pad_char;
  data.pin1.min_length = pin->min_length;
  data.pin1.max_length = pin->max_length;
  data.pin1.pad_length = pin->stored_length;
  data.pin2.data       = newpin;
  data.pin2.len        = (int)newpinlen;
       ...
       const u8 *newpin, size_t newpinlen)
{
  ...
  /* CLCO 21/05/2010 : si le code PUK n'est pas founi, ne pas le revalider */
  if (puk && (r = _validate_pin(p15card, puk_info, puklen)) != SC_SUCCESS)
  /* CLCO 21/05/2010 : fin */
    return r;
  ...
  /* CLCO 09/06/2010 : Gestion du nombre d'essais restants */
  if (!r) /* si pas d'erreur, il faut remettre le compteur à sa valeur max */
    pin->tries_left=pin->tries_max;
  else if (r == SC_ERROR_AUTH_METHOD_BLOCKED)
    pin->tries_left=0;
       ...
       struct sc_pkcs15_pin_info *pin)
{
  ...
  /* indiquer la référence du pin */
  pin_counter.pin_reference=pin->reference;
  r = sc_get_pin_counter(card, &pin_counter);
  if (r)
    goto out;
  pin->tries_left=pin_counter.tries_left;
  pin->tries_max=pin_counter.tries_max;
