...
int sc_disconnect_card(sc_card_t *card, int action)
{
  ...
  ctx = card->ctx;
  SC_FUNC_CALLED(ctx, 1);
  if (card->flags & SC_CARD_STARTUPDATE) {
	  /* Dans le cas particulier de la mise a jour de la carte il faut garder la carte locked */
      /* Si la carte est arrachée durant la phase de mise a jour, il faut la débloquer pour eviter l'assert */
	  if (card->lock_count == 1) sc_unlock(card);
  }
  ...
  sc_file_t **file)
{
  ...
  if (in_path->type == SC_PATH_TYPE_CPS2TER) {
    if (card->ops->cps2ter_select_file == NULL) {
      SC_FUNC_RETURN(card->ctx, 2, SC_ERROR_NOT_SUPPORTED);
    }
    r = card->ops->cps2ter_select_file(card, in_path, file);
  }
  else {
...
int sc_select_cached_file(sc_card_t *card, const sc_path_t *in_path, sc_file_t **file, int use_cache)
{
  int r;
  ...
  if (use_cache == USE_CACHE) {
    r = sc_pkcs15_read_cached_select_file(card, in_path, file);
  }

  if (r) {
    r = sc_select_file(card, in_path, file);
    /* MCUG 14/09/2010 : Gestion de la mise à jour des fichiers de situations */
    if ((use_cache == USE_CACHE || use_cache == MAJ_CACHE) && !r && file && *file) {
      r = sc_pkcs15_select_cache_file(card, in_path, *file);
    }
  }
  return r;
}
